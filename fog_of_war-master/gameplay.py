...
from prepare import GFX, SCREEN_RECT,EYESIGHT
...
class Gameplay(GameState):
    def __init__(self):
        ...
        self.outer = pg.sprite.Group()
        self.wolves = pg.sprite.Group()
        for _ in range(10):
            pos = randint(32, w - 32), randint(32, h - 32)
            Wolf(pos,self,self.wolves, self.all_sprites,self.outer)
        self.trees = pg.sprite.Group()
        for _ in range(25):
            pos = randint(0, w), randint(0, h)
            Tree(pos, self.trees, self.all_sprites,self.outer)
        self.fog = Fog(self,self.all_sprites)
    ...
    def check_wolf_visible(self):
        ...
        light_area.rect = light_area.surface.get_rect(center = self.player.rect.center)
        wolves_list = pg.sprite.spritecollide(light_area,self.wolves,False)
